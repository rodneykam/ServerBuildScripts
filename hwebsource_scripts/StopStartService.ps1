...
<#
.SYNOPSIS
Performs stop or start of services suplied uning the -serviceList flag
.Description
-StopAllMode will stop all services in list 
-StartAllMode will start all services in list and if disable will enable the service and get it started
-CheckAlRunMode will give a report of all services that were started
-CheckAllStopMode will give a report of all services that were stopped
-DisableServices used together with stop and restart will disable the service
.Example
./StopStartService.ps1 -StopAllMode -serviceList "SMTPSVC,IISADMIN,METASCAN"
./StopStartService.ps1 -StopAllMode -DisableServices -serviceList "SMTPSVC,IISADMIN,METASCAN"
#>
# Change to the DeployHelp directory - this allows the script to be run remotely through WinRM
#pushd E:\RelayHealth\DeployHelp
write-host -f green "Entering StopStartSMTP.ps1";
...
function Stop-Services($services)
{
	...
	if ($services -ne $null)
	{
		foreach ($serviceDefinition in $services)
		{
			...
			$service = GetServiceByName $serviceDefinition
			...
			#CheckServiceDependency -service $service -dependentOnServiceName $serviceDefinition
		}
		...
		foreach ($serviceDefinition in $services)
		{
		                     $disabled  = gwmi win32_service | ? { $_.name -eq $serviceDefinition -and  $_.startmode -ne 'disabled'}
							 $disabled
		                   if($disabled )
						     {
							 write-host ".... $($disabled.name)"
		                       $disabled | % -process { SC.EXE config $disabled.Name start= disabled}
							   sleep 2
		                     }
		}					 
		}	
...
function Chk-ServiceEnabled([string] $svcs)
{
	...
	if ($svcs -ne $null)
	{
		...
		{	
			        ...
			        if($enabled)
					        {
					        Write-Host $disabled.name "starupmode was changed from disabled to " $enabled.startmode -NoNewline -ForegroundColor Cyan	
					        }
			 }else{ Write-Host $disabled.name "starupmode was changed from disabled to " $enabled.startmode -NoNewline -ForegroundColor Cyan	}
...
write-host -f green "Starting StopStartService.ps1";
...
if($MaintenanceMode)
{
	...
	write-host -f green "Stopping Services";
...
elseif($StartAllMode)
{
	...
	write-host -f green "Starting services";
...
{
	###START ALL RELAY WINDOWS SERVICES AND CHANGE SCHEDULED TASK ENABLED###
	write-host -f green "Checking services";
	Confirm-ServicesWereStarted $services
	#RunScheduledTask
	#ChangeScheduledTask ENABLE
}
...
#popd
write-host -f green "End StopStartService.ps1";
