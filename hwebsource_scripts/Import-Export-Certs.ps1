...
#[System.Security.SecureString]	$Password = ConvertTo-SecureString $Password
...
$svrname = "healthvault.relayhealth.com.pfx"
$certsPath = "E:\config"
$Certs = @()
$Certs = @("healthvault.relayhealth.com.pfx")

#Set-EncryptedData -key $key -plainText "6Bw+t7-MG*q9.4" > 

foreach($cer in $Certs){

$certPwd = ConvertTo-SecureString $password -AsPlainText -Force
$certPath = "{0}\{1}" -f $certsPath, $cer

Import-Certificate -Path $certPath -Password $certPwd -Storage "LocalMachine" -Container "My"
