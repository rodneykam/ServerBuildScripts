Function GetAdmin([ref]$isAdmin)
{
	...
	if($isAdmin.value)
		{ Write-Host -Foregroundcolor Yellow "GetAdmin function - Current console has admin rights `n" } # [thartzler] added function name
	else
		{ Write-Host -Foregroundcolor Red "GetAdmin function - Current console does not have admin rights. `n" # [thartzler] changed to red
		exit
		}
}
